...
const ImagenGenerator: React.FC = () => {
  ...
  const [params, setParams] = useState({ prompt: '', model: 'imagen-4.0-fast-generate-001', ratio: '1:1' });

  // Filter jobs khusus Imagen
  const imagenJobs = imageJobs.filter(j => j.id.startsWith('im_'));
  ...
  const handleRefinePrompt = async () => {
    ...
    try {
      ...
      if (response.data.refined) {
        ...
        toast.success("Prompt Imagen Diperhalus!");
      }
    } catch (e) {
  ...
  const handleDownload = async (imgUrl: string) => {
    ...
    try {
      ...
      a.href = blobUrl;
      a.download = `Viintools_Imagen_${Date.now()}.png`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const storeJobId = `im_job_${Date.now()}`;
    ...
    try {
      const idToken = await auth.currentUser?.getIdToken();
      const res = await axios.post('/api/imagen', params, {
        headers: { Authorization: `Bearer ${idToken}` }
      });
      ...
      if (res.data.ok) {
        ...
        toast.info("Permintaan Imagen sedang dikirim...");
      } else {
        throw res.data;
      }
    } catch (e: any) {
      removeImageJob(storeJobId);
      if (e.limit_reached || e.response?.status === 429) {
        setShowUpgradeModal(true);
      } else {
        toast.error(e.message || "Generasi Imagen gagal.");
      }
    } finally {
  ...
  return (
    <div className="max-w-[500px] mx-auto pb-20 space-y-10">
      <div className="space-y-2 px-2">
        <h1 className="text-3xl font-black font-heading italic uppercase text-white leading-none">IMAGEN <span className="text-[#3BF48F]">AI</span></h1>
        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest opacity-60">Google Imagen 4.0 Engine</p>
      </div>
      ...
      <div className="glass p-8 rounded-3xl border-white/20 space-y-6">
        <div className="relative group">
          <textarea
            ...
            placeholder="Deskripsikan visi Anda (cth: Animasi 3d kucing Persia)..."
            value={params.prompt}
        ...
        >
          {isGenerating ? <div className="flex items-center justify-center gap-2"><RefreshCw className="animate-spin" size={18} /> Sedang Memproses...</div> : 'Buat Gambar Imagen'}
        </button>
      </div>

      <div className="space-y-4">
        <div className="flex items-center gap-2 px-2 text-white/50"><LucideImage size={16} /><h3 className="text-xs font-black uppercase tracking-widest">Riwayat Imagen</h3></div>
        <div className="grid grid-cols-1 gap-6">
          {imagenJobs.slice(0, 10).map(job => (
              ...
              {job.status === 'processing' || job.status === 'pending' ? (
                <div className="aspect-square flex flex-col items-center justify-center space-y-4">
                  ...
                  <div className="text-center space-y-1">
                    <p className="text-[10px] font-black text-white uppercase tracking-widest">Imagen Proses</p>
