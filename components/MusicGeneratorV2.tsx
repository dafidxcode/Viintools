...
const MusicGeneratorV2: React.FC = () => {
    ...
    const [params, setParams] = useState({
        ...
        style: '',
        prompt: '',
        model: 'V3', // Default for V2 Generator (using V3 endpoint)
        vocalGender: 'm',
    });

    const [jsonInput, setJsonInput] = useState('');

    // V3 typically supports longer prompts
    const promptLimit = activeMode === 'custom' ? 3000 : 1000;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let finalPayload: any = { type: 'music', useV3: true }; // Enforce V3
        ...
        const storeJobId = `mjob_v2_${Date.now()}`;
        addJob({ id: storeJobId, status: 'pending', createdAt: Date.now(), parameters: finalPayload });

        try {
            const idToken = await auth.currentUser?.getIdToken();
            // Using generic /api/generate but payload has useV3: true
            ...
            else toast.error("Gagal menghubungi neural engine V2.");
        } finally {
    ...
    return (
        <div className="max-w-[500px] mx-auto pb-20 space-y-10">
            <div className="space-y-2 px-2 text-center md:text-left">
                <h1 className="text-3xl font-black font-heading italic uppercase text-white leading-none">MUSIK <span className="text-[#3BF48F]">AI V2</span></h1>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest opacity-60">Neural V3 High Fidelity Engine</p>
            </div>
            ...
            <div className="glass p-8 rounded-3xl border-white/20 space-y-6">
                <div className="space-y-2">
                    ...
                    <select className="w-full bg-[#090B14] border border-white/20 rounded-xl px-4 py-3 text-xs font-black text-[#3BF48F] uppercase outline-none focus:ring-1 focus:ring-[#3BF48F]/50" value={params.model} onChange={e => setParams({ ...params, model: e.target.value })}>
                        <option value="V3">SUNO V3 (RECOMMENDED)</option>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    {activeMode === 'simple' && (
                        <div className="space-y-2">
                            <div className="relative group">
                                <textarea rows={4} className="w-full bg-[#090B14] border border-white/20 rounded-xl px-6 py-5 pr-14 text-sm text-white focus:border-[#3BF48F]/50 outline-none resize-none transition-all placeholder:text-slate-700" placeholder="Deskripsikan lagu (V2 supports longer prompts)..." value={params.prompt} maxLength={1000} onChange={e => setParams({ ...params, prompt: e.target.value })} />
                    ...
                    {activeMode === 'json' && (
                        <div className="space-y-4">
                            <textarea rows={8} className="w-full bg-[#090B14] border border-white/20 rounded-xl px-6 py-5 text-xs font-mono text-white focus:border-[#3BF48F]/50 outline-none resize-none transition-all placeholder:text-slate-700" placeholder='{"prompt": "...", "style": "..."}' value={jsonInput} onChange={e => setJsonInput(e.target.value)} />
                        </div>
                    )}
                    ...
                    <button type="submit" disabled={isGenerating || activeJobsCount >= 2} className="w-full py-4 rounded-xl font-black text-sm bg-[#3BF48F] hover:bg-[#2ED079] text-[#090B14] transition-all uppercase italic tracking-tighter shadow-lg shadow-[#3BF48F]/20 disabled:opacity-50">
                        {isGenerating ? <div className="flex items-center justify-center gap-2"><RefreshCw className="animate-spin" size={18} /> Memproses (V2)...</div> : 'Buat Musik (V2)'}
                    </button>
                </form>
            </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest">{job.status} {job.parameters?.useV3 ? '(V2)' : ''}</p>
                                </div>
                            </div>
...
export default MusicGeneratorV2;
